...
def convert_timestamp_to_utc(epochs):
  return datetime.fromtimestamp(\
    epochs, timezone.utc).strftime("%m/%d/%Y, %H:%M:%S")

def convert_timestamp_to_time_at_latlong(timestmp, lat, long):
  utc_time = datetime.strptime(\
    convert_timestamp_to_utc(timestmp), "%m/%d/%Y, %H:%M:%S")
...
def compute_time_of_fly_over(lat, long, iss_fly_over_function):
  try:
   return convert_timestamp_to_time_at_latlong(
    iss_fly_over_function(lat, long), lat, long)
