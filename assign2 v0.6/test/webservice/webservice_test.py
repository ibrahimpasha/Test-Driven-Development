...
class Webservice(unittest.TestCase):
  ...
  def test_fetch_iss_fly_over_data_and_calls_service_and_parse_response_to_parse_json_data(self, mock_parse, mock_get_url):
    mock_get_url.return_value = 'mock_data'
    mock_parse.return_value = 1569953945
    
    fetch_iss_fly_over_data(40.73061, -73.935242)

    self.assertTrue(mock_get_url.called)
    mock_parse.assert_called_with(mock_get_url.return_value)
  ...
  def test_fetch_iss_fly_over_data_and_returns_error_returned_by_parse_json_data(self, mock_parse, mock_get_url):
    mock_parse.side_effect = Exception('Latitude must be number between -90.0 and 90.0')
    mock_get_url.return_value = 'mock_data'
    mock_get_url.return_value = 1569954053
